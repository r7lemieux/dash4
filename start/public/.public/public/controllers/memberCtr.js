...
angular.module('dash').controller('MemberCtr', function ($scope, $q, $stateParams, $location, Member, listSrv) {
      ...
      $scope.initMemberList = function () {
        var membersReady = $q.defer();
        membersReady.promise.then(function (mems) {
          $scope.members = mems;
        });
        //$scope.members = membersReady.promise;
      ...
      var columnDefs = [
        {
          ...
          cellTemplate  : '<div class="ngCellText photo"><a ng-href="#/members/{{row.entity.id}}"><img ng-src="{{row.entity.photo.photo_link}}"/></a></div>'
        },

        {
          ...
          width         : 300,
          enableCellEdit: false,
          cellTemplate  : '<div class="ngCellText"><a ng-href="#/members/{{row.entity.id}}">{{row.entity.name}}</a></div>'
        },
        {
          name          : 'Date Joined',
          field         : 'date',
